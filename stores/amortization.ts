...
export const useAmortizationStore = defineStore('amortization', {
  actions: {
     ...
     */
    calculateEMI (principal: number, rate: number, periods: number): number {
      if (rate === 0) {
    ...
    generateSchedule (
      ...
      const emi = this.calculateEMI(principalAmount, periodRate, numberOfPayments)
    ...
    calculateTotalInterest (
      ...
      const schedule = this.generateSchedule(principalAmount, interestRateMonthly, tenureMonths, paymentFrequency)
    ...
    calculateTotalAmount (
      ...
      const totalInterest = this.calculateTotalInterest(principalAmount, interestRateMonthly, tenureMonths, paymentFrequency)
