...
export const useLoans = () => {
  ...
  // Computed properties
  const pendingApprovalLoans = computed(() =>
    loans.value.filter(loan => loan.status === 'pending_approval')
  )
  ...
  const overdueLoans = computed(() =>
    loans.value.filter(loan =>
      loan.status === 'active' && new Date(loan.end_date) < new Date()
    )
  )

  const closedLoans = computed(() =>
    loans.value.filter(loan => loan.status === 'closed')
  )

  const rejectedLoans = computed(() =>
    loans.value.filter(loan => loan.status === 'rejected')
  )
